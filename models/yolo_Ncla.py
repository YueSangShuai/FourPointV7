    ...
    def forward(self, x):
        ...
        for i in range(self.nl):
            ...
            if not torch.onnx.is_in_onnx_export():
                ...
                x_det, x_kpt = x[i].split((5 + self.nc, int(self.no) - 5 - self.nc), -1)
            ...
            if not self.training:  # inference    True
                ...
                if self.inplace:  # True
                    ...
                    if self.nkpt != 0:
                        ...
                        x_kpt_x = (x_kpt_split * 2. - 0.5 + kpt_grid_x.repeat(1, 1, 1, 1, self.nkpt)) * self.stride[
                            i].item()  # xy
                        x_kpt_y = (y_kpt_split * 2. - 0.5 + kpt_grid_y.repeat(1, 1, 1, 1, self.nkpt)) * self.stride[
                            i].item()  # xy
